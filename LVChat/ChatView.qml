...
Item {
    ...
    //按住说话按钮的实现：为了区别按下和抬起的状态，通过onPressed和onReleased信号来实现按下录音、抬起停止录音并发送消息的效果
    Rectangle {
        id: pressTalk;
...
//ListView显示语音消息，左侧是收到的消息，右侧是发出的消息
    ListView {
        id: chatList;
        anchors.left: parent.left;
        anchors.right: parent.right;
        ...
        anchors.margins: 4;
        clip: true;  //设置为true时ListView内的语音消息就不会越界显示在“按住说话”按钮的下面了
        spacing: 6;
        model: ListModel {}
        //delegaste根据消息来源(direction为0表示发出，否则为收到),决定如何放置头像和气泡背景,气泡背景使用BorderImage
        delegate: Item{
            width: parent.width;
            ...
            //处理消息根据自定义pressed的值来改变图源，这样客户手指按下时消息就会变化
            BorderImage {
                id: msgWrapper;
